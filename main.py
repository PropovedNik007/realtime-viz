...
ds = xr.open_mfdataset(data_path, chunks=None, parallel=False)
...
# ds = xr.open_mfdataset(filtered_files, chunks={"time": 1}, parallel=True)
# ds = xr.open_mfdataset(filtered_files, chunks={"time": 1, "lat": 100, "lon": 100}, parallel=True)


# filtered_data = ds[emission_type].sel(time=slice(start_date, end_date))
filtered_data = ds[emission_type]

# if show_daily and daily_date_range:
#     filtered_data = filtered_data.sel(time=daily_date_range)
# else:
#     filtered_data = filtered_data.sel(time=slice(start_date, end_date))
...
if filtered_data.time.size > 0:
    try:
        if show_daily:
            ...
            data_to_plot = monthly_avg.mean(dim="time").compute()
        ...
        # mean_emission_df = data_to_plot.to_dataframe().reset_index()
        ...
        # view_state = pdk.ViewState(
        #     longitude=mean_emission_df["lon"].mean(),
        #     latitude=mean_emission_df["lat"].mean(),
        #     zoom=2,
        #     min_zoom=0,
        #     max_zoom=15,
        #     pitch=40.5,
        ...
        r = pdk.Deck(
            ...
            tooltip={"text": "{C}"},
            # tooltip={"text": f"{emission_type}: {{emissions}}"},
        )
